...
func main() {
	...
	questioner.POST("/spectator/display", sendSpectatorDisplay)
	questioner.POST("/resetAll", resetAll)

	// // spectator
	// spectator := game.Group("/spectator")
	// spectator.POST("/display", sendSpectatorDisplay)
...
func sendSpectatorDisplay(c echo.Context) (err error) {
	req := new(SpectatorDisplay)
	if err = c.Bind(req); err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
	if err = c.Validate(req); err != nil {
	...
	broadcastMessageToPlayers("question_spectator", req)
	return c.JSON(http.StatusOK, req)
}
